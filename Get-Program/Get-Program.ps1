...
.DESCRIPTION
This function generates a list by querying the registry and returning the installed programs of the computer.

.NOTES   
Name       : Get-Program
Author     : Christian Erb
Version    : 1.0.0
DateCreated: 2023-03-01
License: MIT

Original author: Jaap Brasser
Original name: Get-RemoteProgram
...
.EXAMPLE
Get-Program -Property DisplayVersion,VersionMajor

Description:
Will gather the list of programs and attempts to retrieve the displayversion and versionmajor subkeys from the registry for each installed program

.EXAMPLE
Get-Program -Property Uninstallstring

Description
Will retrieve the installed programs and retrieves the uninstall string for each program

.EXAMPLE
Get-Program -Property Uninstallstring -ExcludeSimilar -SimilarWord 4

Description
Will retrieve the installed programs and also retrieves the uninstall string for each program. Will only display a single entry of a program of which the first four words are identical.

.EXAMPLE
Get-Program -Property installdate,uninstallstring,installlocation -LastAccessTime | Where-Object {$_.installlocation}

Description
Will gather the list of programs and retrieves the InstallDate,UninstallString and InstallLocation properties. Then filters out all products that do not have a installlocation set and displays the LastAccessTime when it can be resolved.

.EXAMPLE
Get-Program -Property installdate -IncludeProgram *office*

Description
Will retrieve the InstallDate of all components that match the wildcard pattern of *office*

.EXAMPLE
Get-Program -Property installdate -IncludeProgram 'Microsoft Office Access','Microsoft SQL Server 2014'

Description
Will retrieve the InstallDate of all components that exactly match Microsoft Office Access & Microsoft SQL Server 2014

.EXAMPLE
Get-Program -Property installdate -IncludeProgram '*[10*]*' | Format-Table -Autosize > MyInstalledPrograms.txt
...
.EXAMPLE
Get-Program -IncludeProgram ^Office -ProgramRegExMatch
...
process {
    'LocalMachine', 'CurrentUser' | ForEach-Object {
        ...
        $RegBase = [Microsoft.Win32.RegistryKey]::OpenBaseKey([Microsoft.Win32.RegistryHive]::$_,$RegView)
                            ...
                            if ($LastAccessTime) {
                                ...
                                if (-not [string]::IsNullOrEmpty($InstallPath)) {
                                    ...
                                    if (Get-Command -Name Get-CimInstance -Module CimCmdlets) {
                                        ...
                                        $HashProperty.LastAccessTime = Get-WmiObject @Wmi
